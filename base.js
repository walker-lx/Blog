    ...
    if(typeof args == 'string'){
	    ...
	    if(args.indexOf(' ') != -1){
	    	...
	    	for(var i = 0;i<elements.length;i++){
	    		if(nodes.length == 0)  nodes.push(document);//如果默认没有父节点，就放入document
	    		switch(elements[i].charAt(0)){
		    		case '#':
		    		   childElements = [];//清理掉临时节点，让父节点失效，子节点有效
		    		   childElements.push(this.getId(elements[i].substring(1)));
		    		   nodes = childElements;//保存父节点
		    		   break;
		    		case '.':
		    		   childElements = [];
		    		   for(var j = 0;j < nodes.length;j++){
			    		   	var temps = this.getClass(elements[i].substring(1),nodes[j]);
		...
		if(args != undefined){  //undefined是一个值

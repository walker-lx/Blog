$(function(){
	...
	var login = $('#login');
	var screen = $('#screen');
	login.center(350,265).resize(function(){
		login.center(350,265);
		if(login.css('display') == 'block'){
			screen.lock();
		}
	});
	$('#header .dl').click(function(){
		login.center(350,265).css('display','block');
        screen.lock().animate({
        	attr:'o',
        	target:30,
        	t:30,
	...
	//注册框
	var reg = $('#reg');
	reg.center(650, 550).resize(function () {
		if (reg.css('display') == 'block') {
			screen.lock();
		}
	});
	$('#header .zc').click(function () {
		reg.center(650, 550).css('display', 'block');
		screen.lock().animate({
			attr : 'o',
			target : 30,
			t : 30,
